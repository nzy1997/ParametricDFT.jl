...
The full treatment of batched manifold operations, custom `RiemannianGD`/`RiemannianAdam` optimizers, batched einsum contractions, GPU acceleration via `CUDAExt.jl`, and the training pipeline is in a separate document:
