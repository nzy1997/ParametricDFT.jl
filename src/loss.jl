...
# Cache for frequency weight masks: (m, n) → weight matrix
# The weight only depends on image dimensions, not content.
const _FREQ_WEIGHT_CACHE = Dict{Tuple{Int,Int}, Matrix{Float64}}()

"""
    _get_freq_weights(m::Int, n::Int)

Get (or compute and cache) the frequency weight mask for an m×n image.
"""
function _get_freq_weights(m::Int, n::Int)
    key = (m, n)
    haskey(_FREQ_WEIGHT_CACHE, key) && return _FREQ_WEIGHT_CACHE[key]

    center_i, center_j = (m + 1) ÷ 2, (n + 1) ÷ 2
    max_dist = sqrt((m/2)^2 + (n/2)^2)

    weights = Matrix{Float64}(undef, m, n)
    @inbounds for j in 1:n, i in 1:m
        freq_dist = sqrt((i - center_i)^2 + (j - center_j)^2)
        weights[i, j] = 1.0 + (1.0 - (freq_dist / max_dist) * 0.5)
    end

    _FREQ_WEIGHT_CACHE[key] = weights
    return weights
end
...
function topk_truncate(x::AbstractMatrix{T}, k::Integer) where {T}
    ...
    weights = _get_freq_weights(m, n)
    scores = abs.(x) .* weights
