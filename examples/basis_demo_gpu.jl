# ================================================================================
# Batch GPU Basis Demo: QFT, Entangled QFT, and TEBD Comparison
# ================================================================================
# This example demonstrates batch training with optional GPU acceleration.
# It extends the standard basis_demo.jl with:
#   - Batch training (multiple images per optimization step)
#   - GPU acceleration via CUDA.jl
#   - Configurable Riemannian optimizers
#
# Run with:
#   julia --project=examples examples/basis_demo_gpu.jl              # CPU, batch=1
#   julia --project=examples examples/basis_demo_gpu.jl --batch=8    # CPU, batch=8
#   julia --project=examples examples/basis_demo_gpu.jl --gpu        # GPU, batch=8
#   julia --project=examples examples/basis_demo_gpu.jl --gpu --batch=16 --optimizer=lbfgs
#
# Requirements for GPU:
#   - NVIDIA GPU with CUDA support
#   - CUDA.jl installed: ] add CUDA
# ================================================================================
...
function parse_args()
    ...
    for arg in ARGS
        if startswith(arg, "--batch=")
            batch_size = parse(Int, split(arg, "=")[2])
        end
    end
    # Default batch size for GPU if not specified
    if use_gpu && batch_size == 1
        batch_size = 8
    end
    ...
    for arg in ARGS
        if startswith(arg, "--optimizer=")
            opt_str = lowercase(split(arg, "=")[2])
            if opt_str in ["gd", "gradient_descent", "gradient-descent"]
                optimizer = :gradient_descent
            elseif opt_str in ["cg", "conjugate_gradient", "conjugate-gradient"]
                optimizer = :conjugate_gradient
            elseif opt_str in ["lbfgs", "l-bfgs", "quasi_newton", "quasi-newton"]
                optimizer = :quasi_newton
            else
                error("Unknown optimizer: $opt_str. Use: gd, cg, or lbfgs")
            end
        end
    end
    ...
    dataset = "mnist"
    for arg in ARGS
        if arg in ["mnist", "quickdraw"]
            dataset = arg
        end
    end

    return (use_gpu=use_gpu, batch_size=batch_size, optimizer=optimizer, dataset=dataset)
end

const ARGS_PARSED = parse_args()
const USE_GPU = ARGS_PARSED.use_gpu
const BATCH_SIZE = ARGS_PARSED.batch_size
const OPTIMIZER = ARGS_PARSED.optimizer
const DATASET = ARGS_PARSED.dataset
...
if USE_GPU
    @info "Loading CUDA.jl for GPU acceleration..."
    using CUDA
    if !CUDA.functional()
        error("CUDA.jl loaded but GPU not functional. Check your CUDA installation.")
    end
    @info "CUDA initialized: $(CUDA.name(CUDA.device()))"
end
...
# Training configuration - adjusted for batch training
const NUM_TRAINING_IMAGES = 80   # More images for batch training
const TRAINING_EPOCHS = 10       # Fewer epochs with batching
const STEPS_PER_BATCH = 50       # Steps per batch (was steps_per_image)
...
# Output directory
const OUTPUT_DIR = joinpath(@__DIR__, "BasisDemo_GPU")
...
function main()
    ...
    println("="^100)
    println("       Batch GPU Basis Demo: QFT, Entangled QFT, and TEBD Comparison")
    println("="^100)
    println("\nConfiguration:")
    println("  Device:     $(USE_GPU ? "GPU ($(CUDA.name(CUDA.device())))" : "CPU")")
    println("  Batch size: $BATCH_SIZE")
    println("  Optimizer:  $(optimizer_names[OPTIMIZER])")
    ...
    training_images, test_images, test_labels = if DATASET == "mnist"
        ...
        if !isdir(QUICKDRAW_DATA_DIR)
            error("Quick Draw data directory not found: $QUICKDRAW_DATA_DIR")
        end
    ...
    println("  Entangled QFT:    $(num_parameters(entangled_qft)) parameters")
    println("  TEBD:             $(num_parameters(tebd_default)) parameters")

    # ============================================================================
    # Step 3: Train All Bases with Batch + GPU
    # ============================================================================

    println("\n" * "="^80)
    println("Step 3: Training All Bases (Batch=$(BATCH_SIZE), Device=$(DEVICE))")
    println("="^80)

    total_coefficients = IMG_SIZE * IMG_SIZE
    k = round(Int, total_coefficients * 0.1)

    n_batches = ceil(Int, length(training_images) * 0.8 / BATCH_SIZE)

    println("\nTraining configuration:")
    println("  Images: $NUM_TRAINING_IMAGES ($(round(Int, NUM_TRAINING_IMAGES * 0.8)) train, $(round(Int, NUM_TRAINING_IMAGES * 0.2)) val)")
    println("  Epochs: $TRAINING_EPOCHS | Batches/epoch: $n_batches | Steps/batch: $STEPS_PER_BATCH")
    println("  Batch size: $BATCH_SIZE | Device: $(USE_GPU ? "GPU" : "CPU")")
    println("  Optimizer: $(optimizer_names[OPTIMIZER])")
    ...
    println("  ✓ Saved: trained_qft.json")
    println("  ✓ Saved: trained_entangled_qft.json")
    println("  ✓ Saved: trained_tebd.json")
    ...
    basis_names = [
        "Standard QFT", "Trained QFT",
        "Standard Entangled QFT", "Trained Entangled QFT",
        "Standard TEBD", "Trained TEBD",
        "Classical FFT"
    ]
    ...
    for ratio in COMPRESSION_RATIOS
        ...
        for (basis_name, basis) in bases
            ...
            @printf("  %-25s PSNR: %.2f dB  SSIM: %.4f\n",
                    basis_name, mean(psnr_vals), mean(ssim_vals))
        end
        ...
        @printf("  %-25s PSNR: %.2f dB  SSIM: %.4f\n",
                "Classical FFT", mean(psnr_vals), mean(ssim_vals))
    end
    ...
    # ============================================================================
    # Step 8: Write Summary to Markdown
    # ============================================================================

    summary_path = joinpath(OUTPUT_DIR, "summary.md")

    md_content = """
# Batch GPU Basis Demo Summary
