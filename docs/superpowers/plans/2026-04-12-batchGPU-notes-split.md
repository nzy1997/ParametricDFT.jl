# batchGPU.typ Notes Split Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Split `note/stepbystep.typ` into a conceptual onboarding guide and a standalone `note/batchGPU.typ` covering batched Riemannian optimization + GPU acceleration, updated to match current code.

**Architecture:** Remove Stages 2-3 from `stepbystep.typ`, add a bridge paragraph pointing to `batchGPU.typ`. Create `batchGPU.typ` with 7 sections (Introduction, Notation, Batched Manifolds, Custom Optimizers, Batched Einsum, GPU Acceleration, Training Pipeline) including 3 new CeTZ diagrams and 3 moved diagrams, all reflecting the latest code.

**Tech Stack:** Typst, CeTZ 0.4.2 for diagrams
...
|------|--------|----------------|
| `note/stepbystep.typ` | Modify | Remove Stages 2-3 content, add bridge paragraph |
| `note/batchGPU.typ` | Create | Standalone batched optimization + GPU document |

---

### Task 1: Trim `stepbystep.typ` — Remove Stages 2-3 and Add Bridge

**Files:**
- Modify: `note/stepbystep.typ`

- [ ] **Step 1: Remove Stage 2 content (lines 137-495)**

Replace lines 137-495 (from `= Stage 2: Custom Riemannian Optimizers` through the end of `== Remaining Problem`) with a bridge paragraph. The new content after line 136 should be:

```typst
= Batched Riemannian Optimization and GPU Acceleration

The problems above --- per-tensor manifold operations, no Riemannian Adam, and no GPU support --- motivate building custom batched optimizers. The full treatment of batched manifold operations, custom `RiemannianGD`/`RiemannianAdam` optimizers, batched einsum contractions, GPU acceleration via `CUDAExt.jl`, and the training pipeline is in a separate document:

#link("batchGPU.typ")[`note/batchGPU.typ` --- Batched Riemannian Optimization and GPU Acceleration]
```

- [ ] **Step 2: Remove Stage 3 content (lines 497-536)**

After Step 1 the old Stage 3 (`= Stage 3: GPU Acceleration` through `== When GPU Wins vs CPU`) should also be gone since it was within the deleted range. Verify that the file now flows: ... Problems Encountered table → bridge paragraph → `= Sparse Basis Types`.

- [ ] **Step 3: Verify the resulting structure**

The file should now have these top-level sections in order:
1. `= Prerequisite`
2. `= Get started`
3. `= Code Structure`
4. `= Stage 1: Manopt.jl Baseline` (with subsections: The Approach, What Manopt.jl Provides, Problems Encountered)
5. `= Batched Riemannian Optimization and GPU Acceleration` (bridge — 3 lines)
6. `= Sparse Basis Types`
7. `= Running Examples`
8. `= Tasks`

Compile with:
```bash
cd /home/claude-user/ParametricDFT.jl/note && typst compile stepbystep.typ stepbystep.pdf
```

If `typst` is not installed, just visually confirm the structure is correct by reading the file.

- [ ] **Step 4: Commit**

```bash
git add note/stepbystep.typ
git commit -m "docs: trim stepbystep.typ — remove Stages 2-3, add bridge to batchGPU.typ"
```

---

### Task 2: Create `batchGPU.typ` — Preamble, Introduction, and Notation (Sections 1-2)

**Files:**
- Create: `note/batchGPU.typ`

- [ ] **Step 1: Write the file header, ngate helper, Introduction, and Notation**

Create `note/batchGPU.typ` with:
...
```bash
git add note/batchGPU.typ
git commit -m "docs: create batchGPU.typ — preamble, introduction, notation"
```

---

### Task 3: `batchGPU.typ` Section 3 — Batched Manifold Operations

**Files:**
- Modify: `note/batchGPU.typ`

- [ ] **Step 1: Append Section 3 content**

Append to `note/batchGPU.typ`:
...
#figure(canvas(length: 0.85cm, {
  ...
  line("tensors", "group", stroke: gray, mark: (end: ">", size: 0.2))

  // Two batched groups (right side, split)
  ...
  line("group", (1.5, 0.2), stroke: gray)
  line((1.5, 1.5), "ubatch", stroke: gray, mark: (end: ">", size: 0.2))
  line((1.5, 0.0), "pbatch", stroke: gray, mark: (end: ">", size: 0.2))
  content((2.3, 1.1), text(6pt)[`stack`])
  content((2.3, -0.1), text(6pt)[`stack`])

  // Manifold ops
  rect((3.0, -2.5), (7.0, -1.5), fill: red.lighten(85%), name: "ops")
  content("ops", text(7pt)[`project` / `retract` / `transport`])
  line("ubatch", (7.0, 1.5), stroke: gray)
  line((7.5, 1.5), (7.5, -2.0), stroke: gray)
  line((7.5, -2.0), "ops", stroke: gray, mark: (end: ">", size: 0.2))
  line("pbatch", (7.0, 0.0), stroke: gray)
  line((7.8, 0.0), (7.8, -2.0), stroke: gray)
  line((7.8, -2.0), (7.0, -2.0), stroke: gray, mark: (end: ">", size: 0.2))

  // unstack back
  line("ops", (3.0, -2.0), stroke: blue, mark: (start: ">", size: 0.2))
  line((2.0, -2.0), (2.0, -3.2), stroke: blue)
  rect((-1.5, -4.2), (1.5, -3.2), fill: blue.lighten(85%), name: "tensors2")
  content("tensors2", text(8pt)[$bold(theta)'_1, bold(theta)'_2, dots, bold(theta)'_K$])
  line((2.0, -3.7), "tensors2", stroke: blue, mark: (end: ">", size: 0.2))
  content((0.5, -2.6), text(6pt)[`unstack_tensors!`])

  // Zygote label
  content((-2.5, -0.8), text(7pt, fill: gray.darken(30%), style: "italic")[Zygote needs\ individual tensors])
  content((8.8, -0.8), text(7pt, fill: gray.darken(30%), style: "italic")[Manifold ops\ need batches])
...
```bash
git add note/batchGPU.typ
git commit -m "docs: add batchGPU.typ Section 3 — batched manifold operations"
```

---

### Task 4: `batchGPU.typ` Section 4 — Custom Optimizers
...
```bash
git add note/batchGPU.typ
git commit -m "docs: add batchGPU.typ Section 4 — custom optimizers"
```

---

### Task 5: `batchGPU.typ` Section 5 — Batched Einsum
...
Append to `note/batchGPU.typ`. This section moves the einsum content from the old `stepbystep.typ` (the OMEinsum background, single-image and batched diagrams, loss dispatch) and adds new content for the einsum cache and updated batched MSE:
...
#figure(canvas(length: 0.85cm, {
  ...
  line("input", "hash", stroke: gray, mark: (end: ">", size: 0.2))
  ...
  line("miss", "save", stroke: gray, mark: (end: ">", size: 0.2))

  // Both paths converge
  rect((-1.5, -7.0), (1.5, -6.0), fill: blue.lighten(85%), name: "result")
  content("result", text(8pt)[optimized einsum code])
  line("hit", (-3.0, -4.5), stroke: gray)
  line((-3.0, -4.5), (-3.0, -6.5), stroke: gray)
  line((-3.0, -6.5), "result", stroke: gray, mark: (end: ">", size: 0.2))
  line("save", (3.0, -5.7), stroke: gray)
  line((3.0, -5.7), (3.0, -6.5), stroke: gray)
  line((3.0, -6.5), "result", stroke: gray, mark: (end: ">", size: 0.2))
...
```bash
git add note/batchGPU.typ
git commit -m "docs: add batchGPU.typ Section 5 — batched einsum"
```

---

### Task 6: `batchGPU.typ` Section 6 — GPU Acceleration
...
```bash
git add note/batchGPU.typ
git commit -m "docs: add batchGPU.typ Section 6 — GPU acceleration"
```

---

### Task 7: `batchGPU.typ` Section 7 — The Training Pipeline
...
```bash
git add note/batchGPU.typ
git commit -m "docs: add batchGPU.typ Section 7 — training pipeline"
```
...
**Files:**
- Read: `note/stepbystep.typ`, `note/batchGPU.typ`

- [ ] **Step 1: Verify `stepbystep.typ` structure**

Read `note/stepbystep.typ` and confirm these sections exist in order:
1. `= Prerequisite`
2. `= Get started`
3. `= Code Structure`
4. `= Stage 1: Manopt.jl Baseline`
5. `= Batched Riemannian Optimization and GPU Acceleration` (bridge)
6. `= Sparse Basis Types`
7. `= Running Examples`
8. `= Tasks`

- [ ] **Step 2: Verify `batchGPU.typ` structure**

Read `note/batchGPU.typ` and confirm these sections exist in order:
1. `= Batched Riemannian Optimization and GPU Acceleration` (introduction)
2. `= Notation`
3. `= Batched Manifold Operations`
4. `= Custom Optimizers`
5. `= Batched Einsum`
6. `= GPU Acceleration` (with `<gpu>` label)
7. `= The Training Pipeline`

- [ ] **Step 3: Try to compile both files**

```bash
cd /home/claude-user/ParametricDFT.jl/note
typst compile stepbystep.typ stepbystep.pdf 2>&1 || echo "typst not installed — skip compilation"
typst compile batchGPU.typ batchGPU.pdf 2>&1 || echo "typst not installed — skip compilation"
```

If `typst` is available and compilation succeeds, verify the PDFs look correct. If `typst` is not installed, skip this step — the Typst source has been verified structurally.

- [ ] **Step 4: Final commit if any fixes were needed**

If any compilation errors or structural issues were found and fixed:
```bash
git add note/stepbystep.typ note/batchGPU.typ
git commit -m "docs: fix batchGPU.typ / stepbystep.typ compilation issues"
```
